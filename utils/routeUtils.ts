...
export async function getDirectionsUrl(
  ...
  try {
    ...
    if (!walkData?.location || !walkData.location.latitude || !walkData.location.longitude) return null;
    
    // Generate Google Maps URL
    const origin = `${participant.lastLocation.latitude},${participant.lastLocation.longitude}`;
    const destination = `${walkData.location.latitude},${walkData.location.longitude}`;
