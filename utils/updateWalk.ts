...
import { doc, updateDoc, collection, getDocs, setDoc, Timestamp } from "@react-native-firebase/firestore";
import { UserData, WithId, Participant } from "walk2gether-shared";
...
async function updateParticipants(
  ...
  try {
    ...
    const participantsRef = collection(firestore_instance, `walks/${walkId}/participants`);
    const participantsSnapshot = await getDocs(participantsRef);
    
    // Create a map of existing participants by user ID
    const existingParticipants = new Map<string, {
      id: string;
    ...
    existingParticipants.forEach((participant, userUid) => {
      if (!invitedUserIdSet.has(userUid) && !participant.cancelledAt) {
        const participantRef = doc(firestore_instance, `walks/${walkId}/participants/${userUid}`);
    ...
    for (const userId of invitedUserIds) {
      ...
      const participantRef = doc(firestore_instance, `walks/${walkId}/participants/${userId}`);
