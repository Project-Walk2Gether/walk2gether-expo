...
export default function WalkRoundModal() {
  ...
  const roundsQuery = useMemo(() => {
    ...
    return query(
      collection(require('@react-native-firebase/firestore').default(), `walks/${walkId}/rounds`),
  ...
  const partnerNames = walk.participantsById 
    ? partnerIds.map(id => walk.participantsById?.[id]?.displayName || "Unknown")
    : ["your partner"];
  ...
  return (
    <View flex={1}>
      ...
      <YStack flex={1} padding="$4" alignItems="center" justifyContent="center" space="$4">
        ...
        {activeRound.questionPrompt && (
          <YStack alignItems="center" marginTop="$8">
            <Text fontSize={16} color="$gray11" textAlign="center" marginBottom="$2">
              Discussion Topic:
            </Text>
